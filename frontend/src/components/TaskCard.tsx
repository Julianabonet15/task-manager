...
export default function TaskCard({ task, onDelete, onClick, onRefresh, selected }: TaskCardProps) {
  ...
  return (
    <tr
      ...
      className={`border-b border-gray-100 cursor-pointer transition-colors hover:bg-gray-50 group ${selected ? 'bg-gray-50' : 'bg-white'}`}
    >
              ...
              <div className="absolute left-0 top-7 z-20 bg-white border border-gray-200 rounded-lg shadow-lg py-1 min-w-[120px]">
                {priorityOptions.map(p => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 flex items-center gap-2 ${p === task.priority ? 'font-medium' : ''}`}
                  >
      ...
      <td className="hidden sm:table-cell px-4 py-4 text-xs text-gray-400"> 
